...
-- These lines are crucial for Mason to install and manage language servers like clangd.
-- They must be called before individual lspconfig.SERVER.setup calls.
...
if mason_lspconfig_ok then
  mason_lspconfig.setup({
    ensure_installed = { "clangd" }, -- This ensures clangd is installed by Mason
  })
end
-- --- END MASON SETUP ---
...
-- --- CLANGD SETUP ---
-- Explicitly setting up clangd. Mason-lspconfig often does this for you
-- if you just put it in `ensure_installed`, but explicit setup gives more control.
lsp.clangd.setup({
  capabilities = capabilities,
  cmd = {
    "clangd",
    "--background-index",        -- Improves responsiveness by indexing in the background
    "--clang-tidy",              -- Enable clang-tidy checks for static analysis
    "--completion-style=detailed", -- Provides richer autocompletion information
    "--fallback-style=llvm",     -- Uses LLVM coding style for formatting if no .clang-format is found
    "--header-insertion=iwyu",   -- Recommends correct #includes based on usage
  },
  filetypes = { "c", "cpp", "objc", "objcpp" },
  -- root_dir function helps clangd find the project root and compile_commands.json
  root_dir = lsp.util.root_pattern(
    "compile_commands.json", -- **Crucial for C++ projects!**
    ".git",
    "Makefile",
    "CMakeLists.txt",
    ".clangd",
  ...
  settings = {
    clangd = {
      -- Fallback flags are used if compile_commands.json is not found.
      fallbackFlags = {
        "-std=c++17", -- Example: use C++17 standard
